...
namespace ArmChair.Middleware.Load
{
    ...
    /// </summary>
    public class LoadFromDatabaseAction : IAction<IEnumerable<LoadContext>>
    {
        private readonly CouchDb _couchDb;

        public LoadFromDatabaseAction(CouchDb couchDb)
        {
            _couchDb = couchDb;
        }
        ...
        public Task Execute(IEnumerable<LoadContext> context, Next<IEnumerable<LoadContext>> next)
        {
            ...
            if (item == null || item.LoadedFromCache) return Task.CompletedTask;
